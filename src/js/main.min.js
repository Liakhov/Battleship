...
adminFiels.onclick = function (event) {
  ...
  if (target.tagName != 'TD' || admin.shipsSunk === admin.numShips) return; //if(target.tagName != 'TD' || admin.shipsSunk === admin.numShips || controller.activePlayer != 'admin') return;
...
userFiels.onclick = function (event) {
  var target = event.target;
  if (target.tagName != 'TD' || user.shipsSunk === user.numShips) return; // if(target.tagName != 'TD' || user.shipsSunk === user.numShips  || controller.activePlayer != 'user') return;
  // Проверка на выстрелы в ячейку
